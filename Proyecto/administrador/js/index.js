...
$("#mnuAsignar").click(abrirAsignar);
$("#mnuSolictud").click(abrirSolicitudCitas);
...
$("#mnuListaAsignacion").click(abrirListaAsignacion);
$("#mnuListaReformas").click(abrirListaReformas);
$("#mnuListaReform").click(abrirListaReform);
...
function abrirAsignar() {
    ...
    // Oculto todos los formularios menos este
    $("table").parent("div").hide("normal");
    $("form:not('#frmAsignar')").parent("div").hide("normal");

    // Verifico si ya he cargado el formulario antes
    // if ($('#frmAsignar').size() == 0) {
    if (document.querySelector("#frmAsignar") == null){
        $("<div>").appendTo('#formularios').load("asignarPeon/asignarPeon.html",
            function() {
                $.getScript("asignarPeon/asignarPeon.js");
            });

    } else {
        // Lo muestro si está oculto
        $('#frmAsignar').parent().show("normal");
    }
}
...
function abrirSolicitud() {
    ...
    // Oculto todos los formularios menos este
    $("table").parent("div").hide("normal");
    $("form:not('#frmAbrirSolicitud')").parent("div").hide("normal");

    // Verifico si ya he cargado el formulario antes
    // if ($('#frmModificacionPersona').size() == 0) {
    if (document.querySelector("#frmAbrirSolicitud") == null){
        $("<div>").appendTo('#formularios').load("solicitudCitas/solicitudCitas.html",
            function() {
                $.getScript("solicitudCitas/solicitudCitas.js");
            });

    } else {
        // Lo muestro si está oculto
        $('#frmAbrirSolicitud').parent().show("normal");
    }
}
...
function abrirListaReformas() {

    // Oculto todos los formularios menos este
    $("form").parent("div").hide("normal");
    $("table").parent("div").show("normal");

    $.getScript("listadoReformas/listadoReformas.js");

    
}

function abrirListaReform() {

    // Oculto todos los formularios menos este
    $("form").parent("div").hide("normal");
    $("table").parent("div").show("normal");

    $.getScript("listarReformas/listadoReformas.js");
